...
cat("\n## Standardized Parameter Estimates", fill=TRUE)
cat("Standardized Parameter Estimates with 95% Confidence Intervals")
...
cat("Modification indices greater or equal than 3.84 in descending order. ", fill=TRUE)
...
cat("We consider several model fit indices from the Lavaan Summary to check the goodness of fit of the model. To decide for an acceptable or non-acceptable model, we apply thresholds which are well established in literature. ", fill=TRUE)
```
...
# MI for factor loadings
mi_fl2 <- mi_fl[mi_fl$decision=="*epc:m*",]
if (nrow(mi_fl2)>0){
...
```{r, results="asis", dev="cairo_pdf", eval=eval2}
cat("\n## Model Respecification", fill=TRUE)
...
cat("In our interpretation, we will use the term *variables* for *indicators* or *items*. ")
cat("\\newline",fill=TRUE)    

...
tryCatch({
  ...
  if (sum(!qqcors)==0 || pnorm > 0.01){
    eval3 <- TRUE
    cat("Normality: We assume that the continuous variables are approximately multivariate normally distributed. ", fill=TRUE)
    cat("\\newline",fill=TRUE) 
  } else {
    cat("**Error: The data is not approximately multivariate normally distributed. Maximum Likelihood is not the propper estimation method. You could try to apply suitable transformations to the data before using this app. Other estimation methods for non-normally distributed data will be implemented soon. For consultancy contact: support@statsomat.com**")
  }
}, 
warning=function(w){
  message(cat("Warning: A multivariate normality check of the data could ne be fulfilled. In the sequel we assume that the data is approximately multivariate normally distributed."))
  eval3 <- TRUE
}
)
```  
...
if (length(which(pe_stand2$Check=="Not ok!"))>0){
  ...
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    cat("Considering the goodness-of-fit indices, the model is acceptable. Nevertheless, there exist localized areas of ill or questionable fit which you should take into consideration. Only if supported by theory, you could try to respecify the model by applying (one of the) following recommendations or call for actions. ")
  } else if (step1 %in% c(0,1,12)){

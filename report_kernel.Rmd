...
cat("Considering also other relevant characteristics of the data resp. the model (sample size, number of factors, estimated loadings), ")

if (weightchi2(nrow(df_work),nfac,max(stdload)) %in% c(0.5,1)){
  cat("we assume a reliable chi-square model fit index and include this result in the overall evaluation of the goodness-of-fit.  ")
  } else {
  cat("we do not assume a reliable chi-square model fit index and therefore do not include this result in the overall evaluation of the goodness-of-fit.  ")
}
```
...
if (round(fitMeasures(fit)["rmsea.ci.upper"],2)<0.06){
  cat("The upper limit of the confidence interval of the RMSEA: ",round(fitMeasures(fit)["rmsea.ci.upper"],2),"is smaller than 0.06, suggesting an acceptable model fit. ")
  rmsea <- TRUE
} else {
    cat("The upper limit of the confidence interval of the RMSEA: ",round(fitMeasures(fit)["rmsea.ci.upper"],2),"is greater or equal than 0.06, suggesting a non-acceptable model fit. ")
}
...
```{r, results="asis", dev="cairo_pdf", eval=eval2}
chi2 <- weightchi2(nrow(df_work),nfac,max(stdload))
gof <- c(chi2)
print(gof)
```

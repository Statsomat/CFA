...
if (length(which(pe_stand2$Check=="Not ok!"))>0){
  ...
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    cat("Considering the goodness-of-fit indices, the model is acceptable. Nevertheless, there exist some localized areas of questionable fit. Therefore, the quality of the model is uncertain. Consider the next recommendations. ")
  } else if (step1 %in% c(0,1,12)){
...
cat("**Further Analysis**", fill=TRUE) # Uncertain
 cat("\\newline",fill=TRUE)
cat("There exist (completely standardized) factor loadings which are uncertain. Further human-based judgement or special statistical analysis is recommended to clarify if these uncertain factor loadings are indeed part of the model or could be dropped. For more (chargeable) support contact support@statsomat.com") 
```

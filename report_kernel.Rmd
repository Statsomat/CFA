...
if ((max(stdload)<=0.5) || (max(stdload)<=0.6 && nfac>=8)){
  ...
  if  (round(fit@Fit@test$standard$stat/fit@Fit@test$standard$df,3) < 2){
    cat(". This ratio is smaller than 2 (the cutoff used by Statsomat), suggesting an acceptable model fit. ")
    
  } else {
    cat(". This ratio is greater or equal than 2 (the cutoff used by Statsomat), suggesting a questionable or non-acceptable model fit. ") 
  }
}  
...
cat("\\newline",fill=TRUE)
cat("The Root Mean Square Error of Approximation (RMSEA) is a parsimony correction fit index based on the chi-square absolute model fit index. Overly complex models are penalized. Values can be greater of equal than zero, with values close to zero suggesting an acceptable model fit. ", fill=TRUE)

if (round(fitMeasures(fit)["rmsea"],2)<0.08){
  cat("The RMSEA: ",round(fitMeasures(fit)["rmsea"],2),"is smaller than 0.08, suggesting an acceptable model fit. ")
} else {
   cat("The RMSEA: ",round(fitMeasures(fit)["rmsea"],2),"is greater or equal than 0.08, suggesting a non-acceptable model fit. ")
}
...
rmseap_weight <- 1-0.5*(nfac<=4 && max(stdload)>0.6)
...
if (round(fitMeasures(fit)["srmr"],2)<0.08){
   cat("The SRMR value of ",round(fitMeasures(fit)["srmr"],3),"is smaller than 0.08, suggesting an acceptable model. ")
} else {
   cat("The SRMR value of ",round(fitMeasures(fit)["srmr"],2),"is greater or equal than 0.08, suggesting a non-acceptable model. ")
}
...
cat("In the overall interpretation of the goodness of fit of the model, we consider also additionally aspects of the data and the model, e.g. the sample size, the number of factors or the magnitude of the loadings. By considering these aspects, individual model fit indices are weighted. The weighting procedure is based on Beauducel & Wittmann 2005. ")

if ((max(stdload)<=0.5) || (max(stdload)<=0.6 && nfac>=8)){
  gof <- c(chi2,chi2todf,cfi,tli,rmsea,rmseap,srmr)
  gof_weight <- c(chi2_weight,chi2todf_weight,cfi_weight,tli_weight,rmsea_weight,rmseap_weight,srmr_weight)
  gof_names <- c("Chi2","Chi2toDF","CFI","TLI","RMSEA","RMSEA p-value","SRMR")
  gof_df <- data.frame(gof,gof_weight,row.names=gof_names)
} else {
  gof <- c(chi2,cfi,tli,rmsea,rmseap,srmr)
  gof_weight <- c(chi2_weight,cfi_weight,tli_weight,rmsea_weight,rmseap_weight,srmr_weight)
  gof_names <- c("Chi2","CFI","TLI","RMSEA","RMSEA p-value","SRMR")
  gof_df <- data.frame(gof,gof_weight,row.names=gof_names)
}
kable(gof_df,col.names=c("Goodness of Fit","Weight"),caption="Summary of the Model Fit Indices")

...
knitr::opts_chunk$set(echo = FALSE, comment='', message = FALSE, error = TRUE, 
                      warning=FALSE, fig.width=8, format="markdown", booktabs = T, longtable = T) # reload knitr options
```
...
if ((sum(pvalues_stand > 0.05, na.rm = TRUE)==0) && (sum(abs(estim_stand) < 0.4, na.rm = TRUE)==0)) {
  cat("Let us inspect the section \"Completely Standardized Parameter Estimates\". We remark that the completely standardized factor loadings (column \"est.std\" from the subsection \"Latent Variables\") are all statistically significant at 5% type I error (i.e. p-value <= 0.05). Moreover, in absolute value they are all greater than 0.4. ")
  pe_stand_indic = "Significant and relevant"
  
} else if ((sum(pvalues_stand > 0.05, na.rm = TRUE) > 0)){
  cat("Let us inspect the table \"Completely Standardized Parameter Estimates\". We remark that there exist completely standardized factor loadings (column \"est.std\" from the subsection \"Latent Variables\") which are not statistically significant at 5% type I error (i.e. p-value > 0.05). ")
  ...
  if (nrow(pe_stand_nonsig_nonrel)>0){
    ...
    pe_stand_indic = "Non significant non relevant"
    ...
    pe_stand_indic = "Non significant but relevant"
  }
 
  # Significant but not relevant 
} else if ((sum(pvalues_stand > 0.05, na.rm = TRUE)==0) && (sum(abs(estim_stand) < 0.4, na.rm = TRUE)>0)) {
  cat("Let us inspect the section \"Completely Standardized Parameter Estimates\". We remark that the completely standardized factor loadings (column \"est.std\" from the subsection \"Latent Variables\") are all statistically significant at 5% type I error (i.e. p-value <= 0.05). Nevertheless, (some) completely standardized factor loadings are in absolute value smaller than 0.4. ")
  pe_stand_indic = "Significant but not relevant" 
} 

cat("This cutoff-value is considered in some CFA research areas a magnitude that is substantially meaningful [@brown]. Please consider also your particular research area when applying the results. ")
...
## Colnames 
colnames(pe_stand2) <- c("Latent","Observed","Factor Loading","R-Square x 100","P-Value","Significant?","Relevance","Sign","Check")
...
two <- "R-Square x 100 represents the variance explained by the factor in percentage. "
...
x <- kable(pe_stand2, digits=2, escape = F, linesep = '', caption="Check Completely Standardized Factor Loadings")
footnote(x, number = number) %>%
  kable_styling(x, position = "center", latex_options = c("hold_position", "repeat_header"))

# Get rid of kableExtra 
...
cat("Next, let us inspect the \"Parameter Estimates\" from the section \"Model Fit Summary\". These represent the unstandardized parameter estimates. We start by analyzing the unstandardized factor loadings. ")
...
cat("Finally, we can proceed by interpreting the unstandardized loadings. ")
text <-  function(cols){
  if (is.na(cols[4])){
...
cat("Please reconsider your data and the theory behind. ", fill=TRUE)  
cat("Only if supported by the theory and research behind the data, you could try to update the model by a line with a high modification index and repeat the evaluation. The goodness of fit will be probably increased. Under certain conditions, additional parameters connected to the variable relationships above, could be added to the model specification. ", fill=TRUE)
```  

...
if (mi_indic==TRUE && (nrow(mi_fl2)>0 || nrow(mi_corr2)>0)) eliminations <- TRUE
if (cov_underestimated==TRUE && nrow(pe_stand3)>0) addings <- TRUE 
respecification <-eliminations || addings

if (eliminations==FALSE && addings==FALSE){
...
if ((mi_indic==TRUE && (nrow(mi_fl2)>0 || nrow(mi_corr2)>0)) || (cov_underestimated==TRUE && nrow(pe_stand3)>0)){
...
if (cov_underestimated==TRUE && nrow(pe_stand3)>0){
  cat("**Factors**", fill=TRUE) # given by special pattern in stand. resid matrix
    cat("\\newline",fill=TRUE)
  cat("Following factors contain at least three uncertain completely standardized factor loadings. In some cases, this is an indicator of a model with too few factors. Only if supported by theory, you could try to split and increase the number of factors in the model: ")
  kable(pe_stand3[,1], col.names = NULL, caption="Factors to Split")
...
cat("Final comments: If you have strong, theory-based reasons and decide to respicify the model, we strongly recommend the replication of the CFA in an independent sample. ")
```  

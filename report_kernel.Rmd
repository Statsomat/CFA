...
tryCatch({
  if (length(extract@pta$vnames$lv.x[[1]])>0){
    eval0 <- FALSE
    stop()
}}, error= function(e) {
  stop(safeError("Higher-order models not allowed by this app. "))
})
```
...
if (exists("df_cont")){
  if (exists("modelvars")){
    if (length(modelvars) > length(colnames(df_cont))){
      cat("**Error: Some observed variables are not considered to be continuous. Other CFA methods for non-continuous data will be implemented soon. For consultancy contact: support@statsomat.com**")
      cat("\\newline",fill=TRUE) 
    } else {
      eval2 <- TRUE
    }
  } else if (ncol(df_cont) < ncol(df)){
    cat("**Error: Some uploaded variables are not considered to be continuous. Other CFA methods for non-continuous data will be implemented soon. For consultancy contact: support@statsomat.com**")
    cat("\\newline",fill=TRUE) 
  } else {
    eval2 <- TRUE
  }
} else {
  cat("**Error: The uploaded variables are not considered to be continuous. Other CFA methods for non-continuous data will be implemented soon. For consultancy contact: support@statsomat.com**")
  cat("\\newline",fill=TRUE) 
}
...
cat("We consider some of the model fit indices from the Model Fit Summary section to check the goodness-of-fit of the model. To decide for an acceptable or non-acceptable model, we apply thresholds considered in the References: Brown [@brown], Kline [@kline]. ", fill=TRUE)
```

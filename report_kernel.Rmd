...
modelcolumns <- colnames(df_work) %in% fit@pta$vnames$ov[[1]] # Observed Variables
modeldata <- df_work[,modelcolumns]
complete_rate <- sapply(modeldata, function(col) 1-(sum(is.na(col)) / dim(modeldata)[1])) 
if (length(which(complete_rate < 1)) != 0L){
  cat("**Warning: Following model variable(s) contain missing values:**")
  indic_missings <- TRUE
  miss_var <- names(which(complete_rate < 1)) 
  knitr::kable(miss_var, col.names = "Variable", linesep = '', longtable=T) %>%
    kable_styling(position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header"))
}
```   
...
tryCatch({
  ...
  # Further evaluation
  eval2 <- TRUE
  indic_missings <-  as.logical(indic_missings*eval2)
...
```{r, results="asis", dev="cairo_pdf", eval=TRUE}
...
cat("We proceed by interpreting the unstandardized factor loadings: ")
...
if (sum(rel_transpose$`Omega (Bentler)`<0.6)==0 && sum(rel_transpose$`Omega (McDonald)`<0.6)==0 && sum(rel_transpose$AVE<0.6,na.rm=TRUE)==0){
  cat("In some fields of research, omega values greater or equal than 0.6 and AVE values greater or equal than 0.5 (fulfilled in your case) could be sufficient. ")
} else {

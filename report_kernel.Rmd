...
if (rmseanull < 0.158){
  if (nullmodel == TRUE){
    cat("The chi-square test with respect to the baseline model and the low RMSEA of the baseline model suggest that the average correlation between the variables is not so high. Therefore, the CFI indicates the departure from a presumptive acceptable baseline model. In this case, the CFI index is a poor model fit index and will not be further evaluated. ")
  } else {
    cat("The low RMSEA of the baseline model suggests that the average correlation between the variables is not so high. Therefore, the CFI indicates the departure from a presumptive acceptable baseline model. In this case, the CFI index is a poor model fit index and will not be further evaluated. ")
  }
} else {
...
if (rmseanull < 0.158){
  if (nullmodel == TRUE){
    cat("The chi-square test with respect to the baseline model and the low RMSEA of the baseline model suggest that the average correlation between the variables is not so high. Therefore, the TLI indicates the departure from a presumptive acceptable baseline model. In this case, the CFI index is a poor model fit index and will not be further evaluated. ")
  } else {
    cat("The low RMSEA of the baseline model suggests that the average correlation between the variables is not so high. Therefore, the TLI indicates the departure from a presumptive acceptable baseline model. In this case, the CFI index is a poor model fit index and will not be further evaluated. ")
  }
} else {
...
if (rmseanull < 0.158){
  if (nullmodel == TRUE){
    ...
    cat("As specified above, the low RMSEA of the baseline model suggests that the average correlation between the variables is not so high. Therefore, in the overall goodness of fit evaluation we consider only the chi-square, the RMSEA and the SRMR model fit indices. ")
  }
  ...
  cat("In the overall goodness of fit summary we consider only the model fit indices: Chi-square, RMSEA, SRMR and TLI. ")
  
  if (tli==0){
    cat("The TLI suggests that your model is not sufficiently better than the baseline model. Therefore, we assume that your model is mediocre or non-acceptable and proceed by diagnosing the sources of possible misspecification. ")
    step1<-5 
...
if (chi2 == 0L && srmr==0L && rmsea==0L){
    ...
    cat("The chi-square model fit index and the RMSEA suggest a non-acceptable model fit. On the other hand there is some support for your model from the SRMR and TLI. We assume that your model is non-acceptable and proceed by diagnosing the sources of possible misspecification. ")
    step1<-7
    # A large sample size evtl. decreases the RMSEA and the SRMR indicates that the chi-square effect is not considerable. 
    # Lack of parsimony --> Too many free parameters ? 

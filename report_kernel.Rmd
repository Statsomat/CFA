...
if (nrow(stand_residuals(fit,"pos"))>0){
  cat("Following variable pairs have standardized residuals which are larger or equal than the considered threshold 2.58 [@brown] or correlation residuals which are larger or equal than the considered threshold 0.1 [@kline]. In these cases, the zero-order covariance relationships between the involved variables are probably underestimated: ")
...
if (nrow(stand_residuals(fit,"neg"))>0){
 cat("Following variable pairs have standardized residuals which are smaller or equal than the considered threshold -2.58 [@brown] or correlation residuals which are smaller or equal than the considered threshold -0.1 [@kline]. In these cases, the zero-order covariance relationships between the involved variables are probably overestimated: ")
...
if (nrow(stand_residuals(fit,"pos"))>0 || nrow(stand_residuals(fit,"neg"))>0){
  cat("Depending on the sample size, the size of the misspecification detected by the analysis of the residual covariance resp. correlation matrices can be relevant or could be in practice neglected. This is matter of subject in the next section. ")
}
```  
...
pe_stand <- standardizedsolution(fit2)
pvalues_stand <- pe_stand[pe$op=="=~",7]
estim_stand <- pe_stand[pe$op=="=~",4]
pe_stand_sig_nonrel <- pe_stand[which(pe_stand$pvalue<=0.05 & pe_stand$op=="=~" & pe_stand$est.std<0.4),3]
pe_stand_nonsig <- pe_stand[which(pe_stand$pvalue>0.05 & pe_stand$op=="=~"),3]
pe_stand_nonsig_nonrel <- pe_stand[which(pe_stand$pvalue>0.05 & pe_stand$op=="=~" & pe_stand$est.std<0.4),3]
r2_nonsig <- r2[r2$`R-Square`<0.7,1]

if ((sum(pvalues_stand > 0.05)==0) && (sum(r2$`R-Square`<0.7)==0) && (sum(abs(estim_stand)<0.4)==0)) {
  cat("Let us inspect the section \"Completely Standardized Parameter Estimates\". We remark that the completely standardized factor loadings (column \"est.std\" from the subsection \"Latent Variables\") are all statistically significant at 5% type I error (i.e. p-value <= 0.05). Moreover, in absolute value they are all greater than 0.4 which is considered a magnitude that is substantially meaningful [@brown].")
  cat("Additionally, the minimum R-Square encountered in the section \"Squared Multiple Correlations\" is",min(r2$`R-Square`), " indicating that at least",min(r2$`R-Square`)," of the variance in the observed variables is accounted for by the latent variables which is satisfactory.")
  
} else if ((sum(pvalues_stand > 0.05) > 0)){
  cat("Let us inspect the table \"Completely Standardized Parameter Estimates\". We remark that the completely standardized factor loadings (column \"est.std\" from the subsection \"Latent Variables\") for the variables: ")
  kable(pe_stand_nonsig)
  cat("are not statistically significant at 5% type I error (i.e. p-value > 0.05). ")
  
  if (nrow(pe_stand_nonsig_nonrel)>0){
    cat("Moreover, in this set there exist completely standardized factor loadings which in absolute value are smaller than 0.4 which is considered a magnitude that is substantially meaningful [@brown]: ")
    pe_stand_nonsig_nonrel
  } else {
    cat("Nonsignificant but relevant. Sample size increase?")
  }
...
if ((sum(pvalues_stand > 0.05)==0) && (sum(abs(estim_stand)<0.4)>0)) {
  cat("Let us inspect the section \"Completely Standardized Parameter Estimates\". We remark that the completely standardized factor loadings (column \"est.std\" from the subsection \"Latent Variables\") are all statistically significant at 5% type I error (i.e. p-value <= 0.05). Nevertheless, the completely standardized factor loadings for these variables are in absolute value less than 0.4 which is considered a magnitude that is substantially meaningful [@brown]:")
  pe_stand_sig_nonrel
  cat("Significant but not relevant. ")
...
if ((sum(pvalues_stand > 0.05)==0) && (sum(r2$`R-Square`<0.7)>0) && (sum(abs(estim_stand)<0.4)==0)) {
   cat("Let us inspect the section \"Completely Standardized Parameter Estimates\". We remark that the completely standardized factor loadings (column \"est.std\" from the subsection \"Latent Variables\") are all statistically significant at 5% type I error (i.e. p-value <= 0.05). Moreover, in absolute value they are all greater than 0.4 which is considered a magnitude that is substantially meaningful [@brown]. ")
  cat("Nevertheless, there exist variables, for which the R-Square encountered in the section \"Squared Multiple Correlations\" is smaller than 0.7. ")
  cat("For these variables, less than 70% of the variance is accounted for by the corresponding latent variables which is in our interpretation non-satisfactory: ")
  kable(r2_nonsig)

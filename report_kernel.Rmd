...
if ((sum(pvalues_stand > 0.05, na.rm = TRUE)==0) && (sum(abs(estim_stand) < 0.4, na.rm = TRUE)==0)) {
  ...
  if (nrow(pe_stand_nonsig_nonrel)>0){
    cat("Moreover, (some) non-significant completely standardized factor loadings are in absolute value smaller than 0.4. This cutoff-value is considered in some CFA research areas a magnitude that is substantially meaningful [@brown]. ")
...
pe_stand2$rel <- NA
pe_stand2$rel[which(pe_stand2$est.std <= 0.4)] <- "!"
pe_stand2$rel[which(pe_stand2$est.std > 0.4)] <- "*"
pe_stand2$rel[which(pe_stand2$est.std > 0.6)] <- "**"
pe_stand2$rel[which(pe_stand2$est.std > 0.8)] <- "***"
colnames(pe_stand2) <- c("","Correlation With Factor","Variance Explained By Factor","","Alpha 5%","")

# Print Interpreted Parameter Estimates
library(kableExtra)
opts <- options(knitr.kable.NA = "")
x <- kable(pe_stand2,digits=3)
add_header_above(x,c("Variable","Factor Loading","R-Square","P-Value","Significance","Relevance"))

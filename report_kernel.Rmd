...
if (nrow(pe_stand2[which(pe_stand2$`P-Value` > 0.05 & abs(pe_stand2$Loading) <= 0.4),])>0){
  ...
  cat("\\newline",fill=TRUE)
  cat("There exist observed variables which are probably not related to the factor(s). Only if supported by theory and further statistical analysis, you could drop the corresponding parameters from the model. ", fill=TRUE) 
}
```

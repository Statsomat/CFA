...
```{r, eval=eval0}
# Higher-order models not allowed 
tryCatch({
  if (length(extract@pta$vnames$lv.nox[[1]])>0){
    eval0 <- FALSE
    stop()
}}, error= function(e) {
  stop(safeError("Non-exogenous latent variables not allowed by this app. "))
})
```
...
if (exists("df_cont")){
  if (exists("modelvars")){
    if (length(modelvars) > length(colnames(df_cont))){
      cat("**Error: Some observed variables are not considered to be continuous. Other CFA methods for non-continuous data will be implemented soon. For support contact: support@statsomat.com**")
      cat("\\newline",fill=TRUE) 
    } else {
      eval2 <- TRUE
    }
  } else if (ncol(df_cont) < ncol(df)){
    cat("**Error: Some uploaded variables are not considered to be continuous. Other CFA methods for non-continuous data will be implemented soon. For support contact: support@statsomat.com**")
    cat("\\newline",fill=TRUE) 
  } else {
    eval2 <- TRUE
  }
} else {
  cat("**Error: The uploaded variables are not considered to be continuous. Other CFA methods for non-continuous data will be implemented soon. For support contact: support@statsomat.com**")
  cat("\\newline",fill=TRUE) 
}
...
tryCatch({
  ...
  if (sum(!qqcors)==0 || pnorm > 0.01){
    ...
    cat("**Error: We cannot assume an approximately (multivariate) normal distribution of the data, which is required in this app. You could try to apply suitable transformations to the data before using this app. Other CFA methods for non-normally distributed data will be implemented soon. For support contact: support@statsomat.com**")
  }
}, 
warning=function(w){
...
cat("We consider some of the model fit indices from the Model Fit Summary section to check the goodness-of-fit of the model. To decide for an acceptable or non-acceptable model, we apply in general thresholds considered in Brown [@brown] and Kline [@kline]. ", fill=TRUE)
```
...
if (length(which(pe_stand2$Check=="Not ok!"))>0){
  ...
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    ...
    cat("Considering the goodness-of-fit indices, the model is non-acceptable. We conclude that the model is non-acceptable. Please reconsider your data and the theory behind. ")
  } else {
    cat("Considering the goodness-of-fit indices, the quality of the model is uncertain. Since we cannot identify (significant and relevant) localized areas of ill fit, we conclude that the model is acceptable but strongly recommend to replicate the CFA in an independent sample to validate the results. ")
  }
  
} else if (respecification==TRUE){
  
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    cat("Considering the goodness-of-fit indices, the model is acceptable. Nevertheless, there exist some localized areas of questionable fit. Please consider the following recommendations or call for actions. If the recommendations are not supported by your research, then we conclude that the model is acceptable. ")
  } else if (step1 %in% c(0,1,12)){
    cat("Considering the goodness-of-fit indices, the model is non-acceptable. Moreover, there exist localized areas of ill fit. Therefore, we assume that the model is non-acceptable. Please reconsider your data and the theory behind. Only if supported by theory, you could try to respecify the model and improve the goodness-of-fit by applying (one of the) following recommendations or call for actions. ")
  } else {
...
cat("There exist (completely standardized) factor loadings which are uncertain. Further statistical tests and/or power and sample size analysis not covered by this app are recommended to clarify their importance. Only supported by theory and further statistical analysis, you could drop the corresponding parameters from the model. ") 
```

```{r, results="asis", dev="cairo_pdf", eval=respecification}
cat("**Recommendations**", fill=TRUE)
```


```{r, results="asis", dev="cairo_pdf", eval=eliminations}
cat("**Eliminations from the Model**", fill=TRUE)
cat("\\newline",fill=TRUE)

# Observed variables
if (nrow(pe_stand2[which(pe_stand2$`P-Value` > 0.05 & abs(pe_stand2$Loading) <= 0.4),])>0){
  cat("**Category Observed Variables**", fill=TRUE) # Not Relevant
 cat("\\newline",fill=TRUE)
...
if (error_covariances_indic==TRUE && nrow(covariances_error)>0){
  cat("**Category Error Covariances**", fill=TRUE) # given by non-sign or < 0.1 residual covariances  
 cat("\\newline",fill=TRUE)
...
if (nfactors_indic==TRUE && intercorr_indic==1){
  cat("**Category Factors**", fill=TRUE)
 cat("\\newline",fill=TRUE)
...
cat("**Addings to the Model**", fill=TRUE)
cat("\\newline",fill=TRUE)
 
if (mi_indic==TRUE && nrow(mi_fl2)>0){ 
  cat("**Category Observed Variables**", fill=TRUE) # given by significant mod indices
 cat("\\newline",fill=TRUE)
...
if (mi_indic==TRUE && nrow(mi_corr2)>0){
  cat("**Category Error Covariances**", fill=TRUE) # given by significant mod indices 
 cat("\\newline",fill=TRUE)
...
if (mi_indic==TRUE && nrow(mi_fcov2)>0){
  cat("**Category Factor Covariances**", fill=TRUE) # given by significant mod indices 
  cat("\\newline",fill=TRUE)

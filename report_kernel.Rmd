...
if (nrow(stand_residuals(fit,"pos"))>0 || nrow(stand_residuals(fit,"neg"))>0){
  cat("Depending on the sample size, the misspecification detected by the analysis of the residual covariance resp. correlation matrices can be relevant or could be in practice neglected. This is matter of subject in the next section(s). ")
}
```  
...
if (nrow(pe_stand2[which(pe_stand2$`P-Value` <= 0.05 & abs(pe_stand2$Loading) <= 0.4),])>0){
  uncertain2 <- "Uncertain. Significant but small(er) effect size. "
} else {
  uncertain2 <- "-----------"
}
...
if (nrow(covariances_error)>0){
  cat("We remark that there exist error correlations which are not statistically significant at 5% type I error or which are are very small and below 0.1 (in absolute value). These error correlations may be unnecessary: ")
  ...
  cat("We remark that all error correlations are above 0.1 (in absolute value) and are statistically significant at 5% type I error. Therefore, we assume that they are well defined. ")
}
...
if (sum(rel_transpose$`Omega (Bentler)`< 0.59)==0 && sum(rel_transpose$`Omega (McDonald)`< 0.59)==0 && sum(rel_transpose$AVE < 0.49, na.rm=TRUE)==0){
  ...
  cat("Nevertheless, omega values below 0.6 or AVE values below 0.5 (at least one of these existent in your case) should be regarded with criticism. ")
}
```  
...
# Check respicification
if (mi_indic==TRUE && (nrow(mi_fl2)>0 || nrow(mi_corr2)>0)) respicification <- TRUE

if (respicification==FALSE){
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    ...
    cat("Considering the goodness-of-fit indices, it is not clear if the model is acceptable or not. Since we cannot identify localized areas of ill fit, we assume that the model is acceptable but strongly recommend to replicate the CFA in an independent sample to validate the results. Consider also a more detailed power and sample size analysis. ")
  }
  
  
  
} else { #respicification
  
  
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    cat("Considering the goodness-of-fit indices, the model is acceptable. Nevertheless, there exist localized areas of ill fit. Therefore, the quality of the model is uncertain. Only if supported by theory, you could try to respecify the model and improve the goodness-of-fit by applying following recommendations. ")
  } else if (step1 %in% c(0,1,12)){
    cat("Considering the goodness-of-fit indices, the model is non-acceptable. Moreover, there exist localized areas of ill fit. Please reconsider your data and the theory behind. Only if supported by theory, you could try to respecify the model and improve the goodness-of-fit by applying following recommendations. ")
  } else {
    cat("Considering the goodness-of-fit indices, it is not clear if the model is acceptable or not. Moreover, there exist localized areas of ill fit. Therefore, we assume that the model is non-acceptable. Please reconsider your data and the theory behind. Only if supported by theory, you could try to respecify the model and improve the goodness-of-fit by applying following recommendations. ")
  }
...
```{r, results="asis", dev="cairo_pdf", eval=respicification}
...
if (mi_indic==TRUE && nrow(mi_fl2)>0){ 
  cat("Update the model by adding cross-loadings to the model indicated by the table \"Significant and Relevant MIs With Respect To Factor Loadings\".", fill=TRUE)
}

cat("**Error Covariances**", fill=TRUE) # given by significant mod indices 
if (mi_indic==TRUE && nrow(mi_corr2)>0){
  cat("Update the model by adding error covariances (correlations) to the model indicated by the table \"Significant and Relevant MIs With Respect To (Residual) Correlations\".", fill=TRUE)
}

if (mi_indic==TRUE){
  cat("Start updating the model by adding a parameter with the largest modification index (and EPC) and only if this parameter can be interpreted meaningfully. Other possible localized are of misfit may be remedied by adding, i.e. freeing a single parameter.", fill=TRUE)
}
  
cat("**Factors**", fill=TRUE) # given by special pattern in stand. resid matrix 

cat("If you respicify the model we strongly recommend the replication of the the CFA in an independent sample. ")

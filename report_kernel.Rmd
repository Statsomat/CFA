...
knitr::opts_chunk$set(echo = FALSE, comment='', message = FALSE, error = TRUE, 
                      warning=FALSE, fig.width=8, booktabs = T, longtable = T) # reload knitr options
```
...
tryCatch({
  ...
  # Parameter estimates for standardized all
...
```{r, results="asis", dev="cairo_pdf", eval=eval3}
cat("\n## Squared Multiple Correlations", fill=TRUE)
pe <- parameterEstimates(fit, rsquare = TRUE)
r2 <- pe[pe$op=="r2",c(1,4)]
colnames(r2) <- c("Variable","R-Square")
rownames(r2) <- NULL
...
```{r, results="asis", dev="cairo_pdf", eval=eval2}
cat("\n## Residuals", fill=TRUE)
cat("We analyze the residual matrices from the Outputs chapter. The residual covariance matrix represents the difference between the sample covariance matrix and the model-implied covariance matrix. Large absolute values indicate local areas of misfit. However, the residuals are affected by the raw metric and are difficult to interpret more precisely. ")
cat("\\newline",fill=TRUE)
...
# Misfit areas 
if (nrow(stand_residuals(fit,"pos"))>0){
  ...
  kable(unique(both), row.names=FALSE, col.names=c("",""), caption = "Pairs with Underestimated Covariance")
} else {
...
if (nrow(stand_residuals(fit,"neg"))>0){
 cat("Following variable pairs have standardized residuals which are smaller or equal than the considered threshold -2.58 [@brown] or correlation residuals which are smaller or equal than the considered threshold -0.1 [@kline]. In these cases, the zero-order covariance relationship between the involved variables is probably overestimated: ")
  ...
  both <- both[order(both[,1],both[,2]),]
  kable(unique(both), row.names=FALSE, col.names=c("",""), caption = "Pairs with Overestimated Covariance")
} else {
...
# No cross-loadings allowed 
# Completely Standardized Factor Loadings
# pe <- parameterEstimates(fit, rsquare = TRUE) # already defined above 
...
pvalues_stand <- pe_stand[pe_stand$op=="=~",7]
estim_stand <- pe_stand[pe_stand$op=="=~",4]
...
if ((sum(pvalues_stand > 0.05, na.rm = TRUE)==0) && (sum(abs(estim_stand) < 0.4, na.rm = TRUE)==0)) {
  cat("We remark that the completely standardized factor loadings (section \"Completely Standardized Parameter Estimates\") are all statistically significant at 5% type I error (i.e. p-value <= 0.05). Moreover, in absolute value they are all greater than 0.4. ")
  pe_stand_indic = "11"
  ...
  if (nrow(pe_stand_nonsig_nonrel)>0){
...
pe_stand2 <- pe_stand[pe$op=="=~",c(1,3,4,7)]
colnames(pe_stand2)[2] <- "Variable"
pe_stand2 <- merge(pe_stand2,r2,by="Variable")
pe_stand2 <- pe_stand2[,c(2,1,3,5,4)]
pe_stand2$`R-Square` = pe_stand2$`R-Square`*100
...
pe_stand2$keep[which(pe_stand2$pvalue > 0.05 & abs(pe_stand2$est.std) > 0.4)] <-  paste0("Not concludent", footnote_marker_number(6, "latex"))
pe_stand2$keep[which(pe_stand2$pvalue <= 0.05 & abs(pe_stand2$est.std) <= 0.4)] <- paste0("Not concludent", footnote_marker_number(7, "latex"))

## Colnames 
colnames(pe_stand2) <- c("Latent","Observed","Factor Loading","Variance Explained","P-Value","Significant?","Relevance","Sign","Check")
names(pe_stand2)[3] <- paste0(names(pe_stand2)[3], footnote_marker_number(1, "latex"))
names(pe_stand2)[4] <- paste0(names(pe_stand2)[4], footnote_marker_number(2, "latex"))
...
two <- "The variance explained is the R-Square x 100 expressed in percentage. "
three <- "5% type I error. "
four <- "Stars correspond to factor loadings cutoff-values: 0.4, 0.6, 0.8."
five <-  "The variable is probably not related to factor. If supported by theory, it could be dropped. "
six <- "If possible, increase sample size and repeat analysis. "
seven <- "Pay special attention to the smaller effect size. Is it good for you? If not, further analysis would be recommended. "
number = c(one,two,three,four,five,six,seven)
...
footnote(x, number = number) %>%
  kable_styling(x, position = "center", latex_options = c("hold_position", "repeat_header"))
...
cat("Next, we inspect the unstandardized factor loadings, contained within the section \"Model Fit Summary\". ")
...
if ((sum(pvalues_loadings > 0.05, na.rm = TRUE)==0)) {
  ...
  if (pe_stand_indic =="11" || pe_stand_indic =="10"){
    cat("Therefore, the significance test results for standardized and unstandardized factor loadings coincide. ")
  }
} else {
...
cat("We summarize the unstandardized factor loadings in the next table: ")
...
## Colnames
colnames(pe2) <- c("Latent","Observed","Factor Loading","P-Value","Significant?","Sign")
...
## Kable
opts <- options(knitr.kable.NA = "")
x2 <- kable(pe2, digits=2, escape = F, linesep = '', caption="Check Unstandardized Factor Loadings", longtable = T)
footnote(x2, number = number2) %>%
  kable_styling(x2, position = "center",latex_options = c("hold_position", "repeat_header"))
options(opts)  # restore global R options
```  

```{r, results="asis", dev="cairo_pdf", eval=pe_unstand_indic}
# Story for Original Parameter Estimates
cat("We summarize the interpretation of the unstandardized factor loadings in the next table: ")
text <-  function(cols){
  if (is.na(cols[4])){
...
story <- apply(pe2, 1, text)
knitr::kable(story, col.names="Interpretation of Unstandardized Factor Loadings",
             caption="Interpretation of Unstandardized Factor Loadings", linesep = '') %>%
  kable_styling(position = "center", latex_options = c("hold_position", "repeat_header"))
```  


```{r, results="asis", dev="cairo_pdf", eval=eval2}
# Extract completely standardized covariances 
covariances_stand <- pe_stand[pe$op=="~~",]
...
cat("**Covariances**")
cat("\\newline",fill=TRUE) 
if (sum(abs(covariances_stand$est.std) > 0.6)==0){
  cat("The largest estimated factor intercorrelation within the section \"Completely Standardized Parameter Estimates\" is",round(max_corr,2),"which we regard as a proof of reasonable discriminant validity. ")
} else {
  cat("Large or statistically significant factor covariances are questionable and provide evidence of poor discriminant validity. There is evidence to question the distinctness of the following factors, since the correlation approaches in absolute value 1.0: ")
  kable(covariances_stand[abs(covariances_stand$est.std)>0.6,])
}
cat("Nevertheless, as noted by [@brown], \"the interpretability of the size and statistical significance of factor intercorrelations depends on the specific research context.\" So, please consider also cutoff-values from your particular research area when interpreting the factor intercorrelations. ")
```  

```{r, results="asis", dev="cairo_pdf", eval=eval2}
cat("**Intercepts**")
cat("\\newline",fill=TRUE) 
cat("Because the data has missing values, a meanstructure i.e. the intercepts of the observed variables are added to the model. The means of the latent factors are fixed to zero. The meanstructure is visible in case of the unstandardized parameter estimates. ")
# No cross-loadings
# Cross-loadings 
```  
...
cat("**Variances**")
cat("\\newline",fill=TRUE) 
cat("Error variances are inversely related to their corresponding factor loadings, i.e. the more variance is explained by a factor, the smaller the error variance is. Completely standardized estimates for error variances represent proportions of unexplained variance. ")
```  


```{r, results="asis", dev="cairo_pdf", eval=eval2}
cat("\n## Final Summary", fill=TRUE)
cat("Please reconsider your data and the theory behind. Only if supported by the theory and research behind the data, you could try to update the model by a line recommended in the section \"Modification Indices \" and repeat the Confirmatory Factor Analysis. Updating by adding a line means adding parameters to the model. The goodness-of-fit will be probably increased. ", fill=TRUE)
...
cat("In our interpretation, we use the term *variables* for *indicators* or *items*. ")
cat("Our interpretation is based mainly on [@brown] and [@kline] and afferent articles. ")   

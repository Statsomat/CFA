...
tryCatch({
  ...
  if (sum(!qqcors)==0 || pnorm > 0.01){
    ...
    cat("**Error: We assume a great departure from multivariate normally distributed. Maximum Likelihood is not the propper estimation method. Other estimation methods for non-normally distributed data will be implemented soon. For consultancy contact: support@statsomat.com**")
  }
}, 
warning=function(w){
  message(cat("Warning: Multivariate normality check of the data could ne be fulfilled. The sequel report assumes that the data is approximately multivariate normally distributed."))
},
finally={
  eval3=TRUE
}

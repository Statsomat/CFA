...
if ((sum(pvalues_stand > 0.05, na.rm = TRUE)==0) && (sum(abs(estim_stand) < 0.4, na.rm = TRUE)==0)) {
  ...
  if (nrow(pe_stand_nonsig_nonrel)>0){
    ...
    cat("Nevertheless, the completely standardized factor loadings are in absolute value greater than 0.4. This cutoff value is considered in some CFA research areas a magnitude that is substantially meaningful [@brown]. Therefore, they are non-significant but they may be relevant. Probably a sample size increase will lead to significance in this matter of subject. ")
...
pe_stand2 <- pe_stand[pe$op=="=~",c(3,4,7)]
colnames(pe_stand2)[1] <- "Variable"
pe_stand2 <- merge(pe_stand2,r2,by="Variable")
pe_stand2 <- pe_stand2[,c(1,2,4,3)]
pe_stand2$`R-Square` = pe_stand2$`R-Square`*100
pe_stand2$pvalue <- sapply(pe_stand2$pvalue,pformat)
## New column
pe_stand2$sig <- NA
pe_stand2$sig[which(pe_stand2$pvalue <= 0.05)] <- "Significant"  
pe_stand2$sig[which(pe_stand2$pvalue > 0.05)] <- "Non-Significant" 
## New column
pe_stand2$rel <- NA
...
## Colnames 
colnames(pe_stand2) <- c("Variable","Factor Loading","R-Square x 100","P-Value","Significance","Relevance","Sign","Check")
names(pe_stand2)[2] <- paste0(names(pe_stand2)[2], footnote_marker_number(1, "latex"))
names(pe_stand2)[3] <- paste0(names(pe_stand2)[3], footnote_marker_number(2, "latex"))
names(pe_stand2)[5] <- paste0(names(pe_stand2)[5], footnote_marker_number(3, "latex"))
names(pe_stand2)[6] <- paste0(names(pe_stand2)[6], footnote_marker_number(4, "latex"))

opts <- options(knitr.kable.NA = "")
x <- kable(pe_stand2,digits=2, escape = F)
...
four <- "The stars correspond to factor loadings cutoff-values: 0.4, 0.6, 0.8."
five <-  "The variable is probably not related to factor. If supported by theory, it could be dropped. "
six <- "If possible, increase sample size. "
seven <- "Pay special attention to the smaller effect size. Further research would be needed to clarify this relationship. "

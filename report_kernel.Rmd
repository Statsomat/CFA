...
if (exists("df_cont") && ncol(df_cont)>0){
...
if (exists("df_cont")){
  if (exists("modelvars")){
    if (length(modelvars) > length(colnames(df_cont))){
      cat("**Error: Some observed variables are not considered to be continuous. Maximum Likelihood is not the propper estimation method. Other estimation methods for non-continuous data will be implemented soon. For consultancy contact: support@statsomat.com**")
      cat("\\newline",fill=TRUE) 
    } else {
      eval2 <- TRUE
    }
  } else if (ncol(df_cont) < ncol(df)){
    cat("**Error: Some uploaded variables are not considered to be continuous. Other estimation methods for non-continuous data will be implemented soon. For consultancy contact: support@statsomat.com**")
    cat("\\newline",fill=TRUE) 
  } else {
    eval2 <- TRUE
  }
} else {
  cat("**Error: The uploaded variables are not considered to be continuous. Other estimation methods for non-continuous data will be implemented soon. For consultancy contact: support@statsomat.com**")
  cat("\\newline",fill=TRUE) 
}
...
if (length(which(complete_rate < 1 & complete_rate > 0.95)) != 0L){
  cat("Missings: There exist variable(s) with <5% missing values. We assume a random missing pattern and apply a missing values imputation technique (FIML). ")
  indic_missings <- TRUE
...
try({
  if (length(knnoutlier(df_cont))>0){
    cat("Outliers: Based on an analysis of the variables considered continuous, we suspect outliers in the data. If observations are erroneous, you could drop them and restart the app. Outliers may affect negatively the execution or the results of the CFA. These are the suspected row numbers: ")
    ...
    cat("Outliers: Based on an analysis of the variables considered continuous, we cannot identify any significant outliers. ", fill=TRUE)
    cat("\\newline",fill=TRUE) 
  }
}, silent=TRUE)
...
tryCatch({
  ...
  if (sum(!qqcors)==0 || pnorm > 0.01){
    ...
    cat("**Error: We cannot assume an approximately (multivariate) normal distribution of the data, which is needed in this app. You could try to apply suitable transformations to the data before using this app. Other methods for non-normally distributed data will be implemented soon. For consultancy contact: support@statsomat.com**")
  }
}, 
warning=function(w){
  cat("Warning: A multivariate normality check of the data could ne be fulfilled. In the sequel we assume that the data is approximately multivariate normally distributed.")
  cat("\\newline",fill=TRUE) 
  cat("Estimation Method: Maximum Likelihood. ", fill=TRUE)
  eval3 <- TRUE
}
...
tryCatch({
  ...
  stop(safeError("Improper CFA solution. This can be due to: Outliers, small sample size, \n multicollinearity, model misspecification etc."))
...
cat("The following table describes the model equations either as entered by you in the text area (denoted by User=1) or established internally (User=0). The last column shows the free parameters which are estimated.")
...
tryCatch({
  ...
  if (length(fit@pta$vnames$lv.x[[1]])>1){
...
if (nrow(mi_corr)>0){
  knitr::kable(mi_corr,col.names=c("Left","Operator","Right","Modification Index", "Expected Parameter Change", "Delta", "Power", "Decision"), digits=3, caption="Modification Indices With Respect To Error Covariances",linesep = '', longtable=T) %>%
    kable_styling(position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header")) %>%
...
if (nrow(mi_fcov)>0){
  knitr::kable(mi_fcov,col.names=c("Left","Operator","Right","Modification Index", "Expected Parameter Change", "Delta", "Power", "Decision"), digits=3, caption="Modification Indices With Respect To Factor Covariances",linesep = '', longtable=T)  %>%
    kable_styling(position = "center", full_width = FALSE, latex_options = c("HOLD_position","repeat_header")) %>%
    footnote("Maximum 10 modification indices in descending order of their magnitude are listed. ")
}
...
cat("We consider some of the model fit indices from the Model Fit Summary section to check the goodness-of-fit of the model. To decide for an acceptable or non-acceptable model, we apply thresholds considered in the References: Brown (Brown 2015), Kline (Kline 2016). ", fill=TRUE)
```
...
if (nrow(stand_residuals(fit,"pos"))>0 || nrow(corr_residuals(fit,"pos"))>0){
  cat("Following variable pairs have standardized residuals which are larger or equal than the considered threshold 2.58 Brown (Brown 2015) or correlation residuals which are larger or equal than the considered threshold 0.1 Kline (Kline 2016). In these cases, the covariance relationship between the involved variables is probably underestimated: ")
  ...
  cat("There are no variable pairs with standardized residuals which are larger or equal than the considered threshold 2.58 Brown (Brown 2015) or correlation residuals which are larger or equal than the considered threshold 0.1 Kline (Kline 2016). Therefore, no relationships among the variables are substantially underestimated by the model. ")
}

if (nrow(stand_residuals(fit,"neg"))>0 || nrow(corr_residuals(fit,"neg"))>0){
 cat("Following variable pairs have standardized residuals which are smaller or equal than the considered threshold -2.58 Brown (Brown 2015) or correlation residuals which are smaller or equal than the considered threshold -0.1 Kline (Kline 2016). In these cases, the covariance relationship between the involved variables is probably overestimated: ")
  ...
  cat("There are no variable pairs with standardized residuals which are smaller or equal than the considered threshold -2.58 Brown (Brown 2015) or correlation residuals which are smaller or equal than the considered threshold -0.1 Kline (Kline 2016). Therefore, no relationships among the variables are substantially overestimated by the model. ")
}
...
cat("This cutoff-value is considered in some CFA research areas a magnitude that is substantively meaningful Brown (Brown 2015). Please consider also cutoff-values from your particular research area when interpreting the factor loadings. ")
...
# Text 
cat("\n### Factor Discriminant Validity", fill=TRUE)
cat("As noted by Brown (Brown 2015), \"the interpretability of the size and statistical significance of factor intercorrelations depends on the specific research context. \"")
...
if (length(which(pe_stand2$Check=="Not ok!"))>0){
  ...
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    ...
    cat("Considering the goodness-of-fit indices, the quality of the model is uncertain. Since we cannot identify (significant and relevant) localized areas of ill fit, we assume that the model is acceptable but strongly recommend to replicate the CFA in an independent sample to validate the results. ")
  }
  
} else if (respecification==TRUE){
  
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    cat("Considering the goodness-of-fit indices, the model is acceptable. Nevertheless, there exist localized areas of ill or questionable fit which you should take into consideration. ")
  } else if (step1 %in% c(0,1,12)){
...
cat("There exist (completely stanardized) factor loadings which are uncertain. Further statistical tests and/or power and sample size analysis not covered by this app are recommended to clarify their importance. Only supported by theory and further statistical analysis, you could drop the corresponding variables from the model. ") 
```
...
if (mi_indic==TRUE && nrow(mi_fcov2)>0){
  cat("**Factor Covariances**", fill=TRUE) # given by significant mod indices 
  cat("\\newline",fill=TRUE)
...
cat("Please consider that this report depends on hard-coded cutoff-values which may be too liberal or too conservative for your research area. The automatic computation and interpretation delivered by the Statsomat app should not completely replace the classical, made by humans graphical exploratory data analysis and statistical analysis. There may be data cases for which the Statsomat does not deliver the most optimal solution or output interpretation.")
```    
...
cat("The statistical analysis was done using R (R Core Team 2019) and following main R packages: lavaan (Rosseel 2012),
corrplot (Wei and Simko 2017), semPlot (Epskamp 2019), semTools (Jorgensen et al. 2020), energy (Rizzo and Szekely
2019), DDoutlier (Madsen 2018).
", fill=TRUE)
```    

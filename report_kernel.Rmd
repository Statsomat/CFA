...
if (nrow(stand_residuals(fit,"pos"))>0){
  ...
  kable(both, col.names=c("",""), caption = "Pairs with Underestimated Covariance")
} else {
...
if (nrow(stand_residuals(fit,"neg"))>0){
 cat("Following variable pairs have standardized residuals which are smaller or equal than the considered threshold -2.58 [@brown] or correlation residuals which are smaller or equal than the considered threshold -0.1 [@kline]. In these cases, the zero-order covariance relationship between the involved variables is probably overestimated: ")
  ...
  both <- unique(both)
  rownames(both) <- paste("Pair",seq(1:nrow(both)))
  kable(both, col.names=c("",""), caption = "Pairs with Overestimated Covariance")
} else {
...
five <- "If possible, increase sample size and repeat analysis. "
...
if (sum(abs(covariances_stand$est.std) > 0.75)==0){
  intercorr_indic = 0
  cat("Though, the largest estimated factor intercorrelation within the section \"Completely Standardized Parameter Estimates\" is",round(max_corr,2),"which we regard as a proof of reasonable discriminant validity. ")
} else {
  ...
  intercorr_indic = 1
  knittr::kable(covariances_stand[abs(covariances_stand$est.std)>0.75,])
}
```  
...
three <- "5% type I error. Typically significant since a large portion of variance is not explained by the latent variable. "
if (cross_loadings(pe_stand)){
  two <- "Corresponds to the squared factor loading. Can be interpreted as proportion of explained variance by the latent factor(s) (%)."
} else {
    two <- "Can be interpreted as proportion of explained variance by the latent factor(s) (%)."
}
footnote <- c(one,two,three)

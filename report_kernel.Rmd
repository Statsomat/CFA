...
tryCatch({
  ...
  stop(safeError("Improper CFA solution. This can be due to: Model misspecification, outliers,\n small sample size, multicollinearity etc."))
...
if (length(which(pe_stand2$Check=="Not ok!"))>0){
  ...
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    ...
    cat("Considering the goodness-of-fit indices, the quality of the model is uncertain. Nevertheless, we cannot identify significant and relevant localized areas of ill fit. If the cutoff-values used by this app to determine significance and relevance and  correspond to your research area, then we conclude that the model is poor but acceptable. In all other cases, we conclude that the quality of the model is uncertain and recommend a human-based CFA. For support contact support@statsomat.com")
  }
  
} else if (respecification==TRUE){
  
  if (step1 %in% c(4,8,9,10,11,14,15,16,17)){
    cat("Considering the goodness-of-fit indices, the model is acceptable. Nevertheless, there exist some localized areas of questionable fit. Therefore, the quality of the model is uncertain. Only if supported by theory, you could try to respecify the model and improve the areas of questionable fit by applying (one of the) following respicification ideas. ")
  } else if (step1 %in% c(0,1,12)){
...
cat("**Further Analysis**", fill=TRUE) # Uncertain
 cat("\\newline",fill=TRUE)
cat("There exist (completely standardized) factor loadings which are uncertain. Further human-based statistical analysis is recommended to clarify their importance. For support contact support@statsomat.com") 
```
...
if (nrow(pe_stand2[which(pe_stand2$`P-Value` > 0.05 & abs(pe_stand2$Loading) <= 0.4),])>0){
  ...
  cat("\\newline",fill=TRUE)
  cat("There exist observed variables which are probably not related to the factor(s). Only supported by theory and further statistical analysis, you could drop the corresponding parameters from the model. ", fill=TRUE) 
}
```

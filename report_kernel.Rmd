...
if (exists("cols_binary")){
  if (sum(cols_binary)>0){
    ...
    kable(names(which(cols_binary==TRUE)), col.names = "Variables considered binary")
  } else {
    cat("0",fill=TRUE)
    cat("\\newline",fill=TRUE) }
}
...
cat("Variables considered norminal and transformed to binary: ", fill=TRUE)
...
cat("\n# Results", fill=TRUE)
fit <- cfa(params$model, data=df_work, std.lv=TRUE, std.ov=TRUE, missing="fiml", estimator="ML", likelihood = "wishart") # to be like AMOS
```     
